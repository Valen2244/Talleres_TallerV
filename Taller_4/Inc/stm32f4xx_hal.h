...
#define HSE_CLOCK_SPEED		4000000				// Valos para la señal de reloj principal (HSE -> High Speed External)
...
/*
 * NOTA:
 * Ahora agregaremos la dirección de memoria base para cada uno de los periféricos que posee el micro
 * En el "datasheet" dek micro, Fig 14 (Memory map) encontramos el mapa de los buses:
 * 	- APB1 (Advance Peripherial Bus)
 * 	- APB2
...
/**
 * AHBx and APBx Bus Peripherials base addresses
 */
...
/**
 * Y ahora debemos hacer lo mismo pero para cada una de las posiciones de memoria de cada uno de los
 * periféricos descritos en la Tabla 1 del manual de referencia del micro.
 * Observe que dicha tabla está a su vez dividida en cuatro segmentos, cada uno de los correspondientes
...
/* Posiciones de memoria para periféricos de AHB2 */
#define USB_OTG_FS_BASE_ADDR		(AHB2_BASE_ADDR + 0x3800U)
 ...
 * periférico RCC.
 *
 * Debido a los temas que se van a manejar en el curso, solo se deben definir los bits de los refistros:
...
typedef struct
{
	...
	volatile uint32_t AHB1RSTR;		// AHB1 Peripherial reset register			ADDR_OFFSET:	0x10
	volatile uint32_t AHB2RSTR;		// AHB2 Peripherial reset register			ADDR_OFFSET:	0x14
	volatile uint32_t reserved0;	// reserved									ADDR_OFFSET:	0X18
	volatile uint32_t reserved1;	// reserved									ADDR_OFFSET:	0X1C
	volatile uint32_t APB1RSTR;		// APB1 peripherial reset register			ADDR_OFFSET:	0x20
	volatile uint32_t APB2RSTR;		// APB2 peripherial reset register			ADDR_OFFSET:	0x24
	volatile uint32_t reserved2;	// reserved									ADDR_OFFSET:	0x28
	volatile uint32_t reserved3;	// reserved									ADDR_OFFSET:	0x2C
	volatile uint32_t AHB1ENR;		// AHB1 peripherial clock enable register	ADDR_OFFSET:	0x30
	volatile uint32_t AHB2ENR;		// AHB2 peripherial clock enable register	ADDR_OFFSET:	0x34
	volatile uint32_t reserved4;	// reserved									ADDR_OFFSET:	0x38
	volatile uint32_t reserved5;	// reserved									ADDR_OFFSET:	0x3C
	volatile uint32_t APB1ENR;		// APB1 peripherial clock enable register	ADDR_OFFSET:	0x40
	volatile uint32_t APB2ENR;		// APB2 peripherial clock enable register	ADDR_OFFSET:	0x44
...
/*
 * Como se vio en la clase de introducción a las estructuras, hacemos un puntero a RCC:RegDef_t que
...
/* ====== FIN de la descipción de los elementos que componen el periférico RCC ====== */

/* +++==== INICIO de la descipción de los elementos que componen el periférico GPIOx ====+++*/
/* Definición de la estructura de datos que representa a cada uno de los registros que componen el
 * periférico GPIO.
 * Debido a que el periférico GPIOx es muy simple, no es necesario crear descripción bit a bit
 * de cada uno de los registros que componen el dicho periférico, pero sí es necesario comprender qué
 * hace cada registro para cargar correctamente la configuración
 */
typedef struct
{
...
/* 8.4.4 GPIO>_PUPDR (dos bit por cada PIN) */
 ...
 * } DUMMY_RegDeg_t;
 */
/* No es necesario hacer nada más en este archivo para el primer proyecto */
